...
passport.use(new GoogleStrategy({
  ...
  const user = await Users.findOrCreate({ where: { googleId: profile.id, fullName: `${profile.name.givenName} ${profile.name.familyName}` }, defaultUser: defaultUser }).catch((err) => {
