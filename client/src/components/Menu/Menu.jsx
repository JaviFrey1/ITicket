...
export default function Menu() {
    ...
    function handleClickCat(id) {
        const subCatOpt = subcategories.filter(subCat => subCat.categoryId === id)
        ...
        <div className={style.contRey}>
            <div className={style.contMenu}> 
                <div className={style.categoriesOptions}>
                    {
                        categories && categories.map(cat => <h3 key={cat.id} onClick={() => handleClickCat(cat.id)}>{cat.name}</h3>)
                    }
                ...
                <div className={style.contSubcategories}>
                    {subCatOpt.map((subCat) => {
                        return <h4 onClick={() => handleClickSubCat(subCat.genre)}>{subCat.genre}</h4>;
                    })}
                </div>
            </div>
        </div>
